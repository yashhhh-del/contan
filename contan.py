...
def display_content_results(results, platform_type):
    ...
    # Action Buttons Row - Regenerate, Edit, Clear All
    st.markdown("""
    <style>
    ...
    with col1:
        regenerate_btn = st.button("🔄 Regenerate", key="regenerate_btn", use_container_width=True)
    
    with col2:
        edit_btn = st.button("✏️ Edit", key="edit_btn", use_container_width=True)
    
    with col3:
        clear_btn = st.button("🗑️ Clear All", key="clear_btn", use_container_width=True)
    ...
    if clear_btn:
        ...
        st.session_state['nlp_keywords'] = None
        st.success("✅ Content cleared successfully!")
        st.rerun()
    
    if edit_btn:
        st.session_state['edit_mode'] = True
        st.info("✏️ Scroll up to edit your inputs and regenerate!")
    
    st.markdown("""
    <div style="background: linear-gradient(135deg, #11998e 0%, #38ef7d 100%); padding: 1.5rem 2rem; border-radius: 16px; margin: 1.5rem 0; text-align: center;">
    ...
    if hashtags:
        ...
        with col_copy:
            ...
            if st.button("📋", key="copy_hashtags", help="Copy Hashtags"):
    ...
    if keywords:
        ...
        with col_copy:
            st.markdown("<div style='height: 20px;'></div>", unsafe_allow_html=True)
            if st.button("📋", key="copy_keywords", help="Copy Keywords"):
...
def render_generate_page():
    ...
    regenerate_trigger = st.session_state.get('regenerate_trigger', False)
    if regenerate_trigger:
        st.session_state['regenerate_trigger'] = False
        generate_btn = True  # Trigger generation
